...
app.use(session({//la configuramos 
    secret: 'unGranSecreto',
    resave: false,//para que no se renueve la sesion
    saveUninitialized: false,//para que no se vuelva a establecer la sesion 
    store: new MySQLStore(database) //aca decimos donde guardar la sesion, en este caso la guardaremos en la BD y para eso toca importar otro modulo, en el argumento le pasamos la conexion de la BD
}));
app.use(flash());//asi podemos usar flash
